...
const initDb = async () => {
	try {
		...
		// Preguntar si se quiere borrar la base de datos
		const promptDBDelete = await inquirer.prompt([
			{
				type: 'confirm',
				name: 'confirm',
				message: '¿Desea borrar la base de datos actual?',
				default: false,
			},
		]);

		if (promptDBDelete.confirm) {
			await pool.query(`DROP DATABASE ${process.env.MYSQL_DB}`);
			console.log('Base de datos borrada ✅\n');
		} else {
			const promptTableDelete = await inquirer.prompt([
				{
					type: 'confirm',
					name: 'confirm',
					message:
						'Estamos a punto de eliminar los datos de las tablas ¿Desea borrar las tablas actuales?',
					default: false,
				},
			]);

			if (promptTableDelete.confirm) {
				...
				console.log(
					'Tabla de Usuarios (`users`) creada o ya existente'
				);
				...
				console.log(
					'Tabla de Contratos (`contracts`) creada o ya existente'
				);
				...
				console.log(
					'Tabla de Reseñas (`reviews`) creada o ya existente'
				);
				...
				console.log(
					'Tabla de Imágenes (`images`) creada o ya existente'
				);
				...
				console.log(
					'Tabla de Favoritos (`favs`) creada o ya existente//n'
				);
				...
				console.log('Tablas de datos ya existentes ✅\n');
				const promptBackUpDump = await inquirer.prompt([
					{
						type: 'confirm',
						name: 'confirm',
						message:
							'¿Desea realizar un dump de la base de datos actual?',
						default: false,
					},
				]);
				if (promptBackUpDump.confirm) {
					await pool.query(`
						CREATE TABLE backup (
							id                     INT UNSIGNED PRIMARY KEY NOT NULL AUTO_INCREMENT,
							userId                 INT UNSIGNED,
